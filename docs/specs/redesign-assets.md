...
> **历史资产快照**：下文版本、资源和补丁判断属于旧实现及盘点时的记录。当前安装状态由 package.json/lockfile 维护，目标插件来源与版本见 [协议规格 §5](protocol-contract.md#5-契约维护与依赖依据)；复用代码和补丁需通过当前行为验收，不因本清单的“预计保留”直接移植。新空库、备份空间和发布方案以 [运维规格](operations.md) 为准，旧资源不作为新部署依赖。
