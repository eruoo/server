...
| 项               | 决策                                                                                            |
| ---------------- | ----------------------------------------------------------------------------------------------- |
| 验证环境         | `eruoo-server-staging` Worker + 独立验证 D1 + workers.dev 域名（生产 workers.dev 保持禁用）     |
| 部署授权         | owner 预授权 M0-M7 的 staging 部署（绝不碰生产）；每步门禁后暂停汇报等确认；M8 生产操作逐次授权 |
| 旧规范地位       | foundation.md 降级为参考资料；新规范在各里程碑定稿，避免双权威                                  |
| GitHub OAuth App | M2 开工前由 owner 创建验证用 App（callback 指向 staging workers.dev 地址），与生产 App 分离     |
