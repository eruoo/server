...
- AI 状态存储切片（§4.14）交付持久状态边界；PR 3–PR 9（§4.15–§4.20）补齐连接器、协议、编排、AI Key 配置档、路由与管理界面，均经本地合成验证。2026-09-19（§4.21）已实测：staging 部署至 `7bf5b8255c1a5b3c9fec406026cbf71b6df2592c`（含 0003 迁移）、真实设备授权通过、首次凭证有效期约 10 天、含 AI 数据的隔离恢复演练通过。**阻断**：模型目录请求返回 403，具体原因尚未确认（出口或边缘防护是待验证假说），模型目录无法建立，真实推理链路（JSON/SSE 调用、在途限制、取消、SSE 分类、断开拒绝、usage、真实令牌刷新）与真实上游事件分类码均未验收，AI 上线门槛未通过，待原因确认并按 owner 决策处理后重跑；`AI_CREDENTIAL_KEYS` 真实轮换、FedRAMP、流内错误分类码集合、令牌长度上限仍待实测。production 未部署、其 `AI_CREDENTIAL_KEYS` 未配置，其实际行为需在部署后单独验收。
