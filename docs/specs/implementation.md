...
| AI 状态存储与凭证生命周期 | 0002 迁移的四张 AI 应用表；连接生命周期、设备授权会话与原子完成、有界刷新 claim 与不确定状态处理、版本约束模型快照、并发 reservation 与 unknown 语义、每日有界清理；恢复规划器按原始表集生成 AI 清理并统一“先清理后迁移”顺序。PR 3（2026-09-19）补齐固定 Codex 连接器、AES-256-GCM 版本化凭证加密（AAD 绑定环境/连接/用途）、设备授权与凭证刷新编排（recent-auth 提交前复核、刷新失败窗口分类）、模型发现与重新授权门槛。AI HTTP 路由与管理界面未开放（Key 配置档已于 §4.18 开放） |
...
本轮按 owner 指令完成最新 staging 发布、0003 迁移、真实 AI 链路验收与含 AI 数据的恢复演练；production 仅整理准备项。此前本机请求 staging `/health` 得到过一次 403：本轮经代理与直连（TUN）复测均返回 200 且响应体为应用 JSON、版本号与部署记录一致，属本机代理链路瞬时现象，不是 Worker 故障，Actions 冒烟同过。
...
**模型发现被上游阻断（本轮主要发现）**：授权完成后首次模型发现失败，归类 `ai-upstream-protocol-error`（该分类同时覆盖 4xx 与载荷不符，当时无状态可见）。按固定参考 codex-rs 目录会话与本服务 `/responses` 编排的既有 401 语义，先修复目录路径缺失的“401 → 强制刷新一次 → 重发一次”（[PR #45](https://github.com/eruoo/server/pull/45)，合并为 `7bf5b8255c1a5b3c9fec406026cbf71b6df2592c`；独立 review approve-with-fixes，三项 minor——事件补 `connectionId`、修正注释覆盖范围、补恢复期预算耗尽与重发失败的组合测试——已全部修复；本地全链 check 通过，worker 431/431、目标文件 36/36）。随后发布 `7bf5b82`（run 35454862688，success，0 迁移，冒烟 5 项，版本 `452ff1f7-b579-4e87-9aec-d33f4e1ceb23`）并重验：**受控日志给出决定性证据 `{"failureKind":"http","httpStatus":403,"reason":"protocol"}`——上游是 403 而非 401，修复正确地未触发恢复。**

403 根因定位（证据链）：同一头型（`originator: eruoo`、`user-agent: eruoo/1`）自本机请求该端点返回 401 JSON（边缘放行、鉴权拒绝）；Worker 出口携有效令牌请求被 Cloudflare 边缘 403 挑战拒绝（令牌未被评估）；公开佐证为 codex CLI Linux/rustls 指纹被同款 403（openai/codex#17860）及 OpenAI 对 Cloudflare Worker 出口 403 封锁的持续社区报告；auth.openai.com（设备码/交换/JWKS）自 Worker 全部正常。owner 确认本机 codex app 与 CLI 均正常——账号与凭证链路无问题，阻断唯一归因于 Worker 出口环境（出口 IP + workerd TLS 指纹不可控）。按 [ai-service §13](ai-service.md#13-脆弱前提与取舍) 该连接器的上线门槛条件触发：不伪造第一方身份、不自动改用付费 API、不新建中继组件；owner 决策为维持现状并记录。AI Key 无法获得目录内模型许可（目录恒为空、创建必带非空 `modelIds`），因此调用链（JSON/SSE 真实调用、在途限制 429、取消收尾、SSE error 分类、断开后拒绝推理、usage 与调用记录）**全部受阻，标为上游阻断未验证**；真实令牌刷新（凭证约 10 天有效，60 秒提前窗口未自然到期）同样未触发。后续验证方法：上游阻断解除后（出口通道变更需 owner 另行决策与授权）按 §4.21 重跑最小闭环。
...
- AI 状态存储切片（§4.14）交付持久状态边界；PR 3–PR 9（§4.15–§4.20）补齐连接器、协议、编排、AI Key 配置档、路由与管理界面，均经本地合成验证。2026-09-19（§4.21）已实测：staging 部署至 `7bf5b8255c1a5b3c9fec406026cbf71b6df2592c`（含 0003 迁移）、真实设备授权通过、首次凭证有效期约 10 天、含 AI 数据的隔离恢复演练通过。**阻断**：chatgpt.com 后端对 Cloudflare Worker 出口请求返回 403 边缘挑战（令牌未被评估；owner 本机 codex 正常），模型目录无法建立，真实推理链路（JSON/SSE 调用、在途限制、取消、SSE 分类、断开拒绝、usage、真实令牌刷新）与真实上游事件分类码均未验证，待出口通道问题按 owner 决策解决后重跑；`AI_CREDENTIAL_KEYS` 真实轮换、FedRAMP、流内错误分类码集合、令牌长度上限仍待实测。production 未部署、其 `AI_CREDENTIAL_KEYS` 未配置。
