> **参考档（v2 起非权威）**：本文档为 v1 规格的历史记录。当前目标由 [完整架构规格及其配套文档](architecture.md#11-唯一维护位置) 维护，入口见 [文档导航](../index.md)；平台历史实测单独见 [platform-facts.md](platform-facts.md)。下文的“单一事实来源”、状态、待办和旧门槛均属于当时记录，不重新定义当前目标；未被 owner 新指令覆盖的外部执行授权边界继续适用。
...
| CI | GitHub Actions | 对 PR 和最终 `main` commit 执行完整门禁，不持有生产凭证 |
| 部署 | Cloudflare Workers Builds | 只监听受保护的 `production`，负责生产 preflight、migration 与 deploy |
...
- 提交 `pnpm-lock.yaml`。本地受控检查、GitHub Actions 和 Workers Builds 均使用 frozen lockfile，构建过程中不得改写依赖解析结果。
...
- 仓库提供稳定的 `format`、`format:check`、`lint`、`lint:fix`、`dependencies:audit`、`typecheck`、`test`、`test:integration`、`test:e2e`、`openapi:check`、`build` 和聚合 `check` 脚本。具体命令只在 `package.json` 维护；本地与 GitHub Actions 调用完整 `check`，Workers Builds 只执行 frozen install 后调用稳定的 `deploy:production` 入口。
...
- 禁用动态客户端注册。
- redirect URI 按登记字符串进行简单字符串比较，不做 URI 规范化。只有 RFC 8252 的 loopback IP 端口可以变化；动态端口必须是 `1` 到 `65535` 的 canonical 十进制，scheme、原始 host 字面量、path、query 与 fragment 均不得借 URL parser 规范化后再匹配，因此 `127.1`、整数/八进制 IPv4、尾点 host、dot segment 和百分号别名都拒绝。
...
- migration 一旦进入 production D1 ledger，对应文件不得修改、重命名或删除；后续修正必须新增 migration。GitHub Actions 必须显式刷新受保护的 `production` tip，要求该精确 commit 是候选 `HEAD` 可读的 first-parent 祖先，并顺序检查完整候选路径。可信状态从 production 快照开始，只有保持旧 migration 名称和字节不变、且新增文件使用更大唯一四位序号的快照才能推进；最终候选必须相对最近合法快照继续追加。后续无关提交不能洗白先前的改写、删除、重命名或序号回填，明确恢复到最近合法状态后可以重新通过。变量缺失、格式错误、祖先关系异常或任一所需 Git 对象无法读取时 fail closed。本地检查没有可信基线变量时只使用直接父提交；Workers Builds 必须明确告警本次检查不具备 append-only 权威性，并只消费已由 GitHub Actions `check` 中的 production-history 门禁与受保护 `production` exact-SHA 门禁通过的同一提交，不维护重复的 checksum 清单。重复执行 apply 时，已应用 migration 必须保持 no-op。
...
- GitHub Actions 的稳定 job 名为 `check`，同时监听指向 `main` 的 PR 和 `main` push；PR run 提供合入前反馈，push run 必须对 squash merge 后的最终 commit 再执行一次完整门禁。Ubuntu runner 固定 Node.js 24.18.0、从 `package.json#packageManager` 读取 pnpm，先执行 `pnpm install --frozen-lockfile` 和 `pnpm exec playwright install --with-deps chromium`，再执行 `pnpm run check`。
- GitHub CI 只授予 `contents: read`，不使用 `pull_request_target`，不读取生产 Secret、Cloudflare token，不运行远端 migration 或 deploy，也不监听 `production`。workflow/job 名称、触发范围、固定工具链、浏览器安装和完整 `check` 由仓库测试守卫。
...
- 默认生产命令保持 strict deploy，未设置一次性 bootstrap 变量时不得执行远端模板探针。
- 构建父环境中的任意 `CLOUDFLARE_API_BASE_URL` 和 `WRANGLER_CI_GENERATE_PREVIEW_ALIAS` 不得阻断入口或进入 Git、preflight、Wrangler、bootstrap inspection 子进程，Wrangler 与 bootstrap inspection 子进程必须固定关闭 Preview Alias，旧别名 `CF_API_BASE_URL` 必须拒绝；bootstrap 直接 API 请求必须始终使用固定 canonical URL。
- `PRODUCTION_WORKER_BOOTSTRAP_VERSION_ID` 只接受无空白的小写 UUID，并锁定远端当前 Dashboard template version；布尔值、空值、大小写或格式错误、遗留变量和执行期间变量变化都必须 fail closed。
- bootstrap 在 migration 前和 deploy 前两次执行最小只读检查：Active Deployment 仍是变量锁定的 Dashboard template，远端 Secret 名称精确等于五个 required Secret，生成配置仍满足核心 identity/binding 契约。每次检查还必须读取只计算、不提交的 Custom Domain changeset，并要求结果只新增 `auth.eruoo.me`，不存在会被替换的 domain、origin 或 DNS record；任一读取失败、冲突或两次检查之间的状态变化都不得执行下一步。
- bootstrap deploy 与默认 deploy 的命令差异只能是省略 `--strict`，并添加由完整 commit SHA 与随机 UUID 构成的唯一 version tag/message；不得添加 `--keep-vars`、`--force`，也不得跳过 preflight、migration、精确 SHA、远端 branch 或 clean worktree 门禁。
- 非 strict deploy 命令无论成功或失败，都必须在最终 Git context 校验前以有界重试做最小远端对账：Active Deployment 来自 Wrangler，具有本次精确 tag/message，不再指向 Dashboard template；当前 version 的 handlers 与核心 bindings、五个 required Secret 必须与生成配置一致，按目标 service/environment 筛选后的 Custom Domain 结果必须且只能是 `auth.eruoo.me`，远端 script settings 不得保留 logpush、Tail Consumer、Streaming Tail Consumer 或外部 logs/traces destination。命令报错即使对账收敛也仍必须标记 build 失败；对账失败必须明确远端可能已经完整或部分写入，不能声称回滚或自动重试。测试必须覆盖 command failure 后仍对账、最终状态不收敛和 eventual consistency 后收敛。
- bootstrap 从首次远端检查到 build 终态必须保持单写者，禁止并发 Dashboard/API/CLI/build 修改 Worker、Secret、route、Custom Domain、cron 或 Workflow。postcondition 是检测边界，不具备 CAS 语义，不能替代排他操作约束。
- 首次接管和生产验收成功后，下一个仓库清理 PR 必须删除 bootstrap 模块、测试、deploy 分支和一次性文档，只保留默认 strict 发布路径；在清理 PR 合入前，临时 Build variable 必须保持不存在。
...
- 使用 GitHub Actions 承担完整 CI，使用 Cloudflare Workers Builds 的 GitHub 集成承担生产部署；Workers Builds 不承担需要额外操作系统依赖安装的浏览器 CI。
- Git 默认分支和日常集成分支为 `main`；Cloudflare Workers Builds 的 production branch 固定为长期分支 `production`，production trigger 只能监听 `production`。同一次 `production` push 不再启动另一套 CI；完整门禁必须已经在相同 commit 的最终 `main` push run 中成功。
- `.github/workflows/check.yml` 只监听指向 `main` 的 PR 和 `main` push，使用稳定且在全仓 workflow 中唯一的 job 名 `check`、只读仓库权限、固定 Node.js、从 `package.json#packageManager` 读取 pnpm、frozen install、Playwright `--with-deps chromium` 与完整 `pnpm run check`。workflow 必须在执行仓库代码前检出完整候选历史，显式刷新 `refs/heads/production` 到固定的 remote-tracking ref，校验其小写非零完整 SHA，并只向 `check` 步骤注入保留变量 `PRODUCTION_MIGRATION_BASELINE_SHA`；`check` 必须实际运行读取真实 migration、该 production commit 与完整候选 first-parent 路径的 `release:preflight`，不能只运行合成 validator 测试。变量缺失、无效、production 不是候选的 first-parent 祖先或任一所需 Git 对象不可读必须 fail closed，非 GitHub Actions 环境不得手工注入，Workers Builds 必须显式拒绝该变量。workflow 不得持有 Cloudflare token、生产 Secret、运行远端 migration/deploy、使用 `pull_request_target`、监听 `production` 或按 path 跳过 required check。
- `production` 必须由一个无 bypass 的 ruleset 要求状态检查 `check` 成功，并把期望来源限定为 GitHub Actions；该门禁检查最终 `main` commit，不能以 PR merge ref、本地检查或人工声明替代。release actor 与历史安全 ruleset 的既有边界继续保留，LoTwT 不得绕过 CI ruleset。
- Workers Builds 的 root directory 为仓库根目录，build command 固定为 `pnpm install --frozen-lockfile`，production deploy command 固定为 `pnpm run deploy:production`。常驻 build variables 固定为与仓库工具链一致的 `NODE_VERSION=24.18.0`、`PNPM_VERSION=11.22.0` 和 `SKIP_DEPENDENCY_INSTALL=1`；最后一项用于禁用平台自动 dependency install，确保依赖只由固定 build command 的 frozen install 安装一次。生产产物检查由 `deploy:production` 的首个 `release:preflight` 步骤执行，不能从发布序列中删除；该门禁必须验证 source/generated 配置中的核心 production 契约，包括固定 account 与 runtime mirror、production D1 ID、唯一 `auth.eruoo.me` Custom Domain route、Static Assets 目录、binding 名称与 ID、migration 配置、required-secret 名称清单，以及显式清空 Streaming Tail Consumer 等外发 telemetry。它不负责枚举 Wrangler 的全部可选字段。
- 默认关闭 non-production branch builds，避免普通功能分支的仓库脚本接触生产级 build token。只有单独建立不含 production D1、Worker、Secret、route 或其他生产写权限的 preview trigger/token 后，才可启用非生产构建，其 non-production deploy command 固定为 `pnpm run bundle:check`，不得上传版本、执行远端 migration 或修改 Worker。
- 根 `package.json` 中的 `deploy:production` 必须先执行 `pnpm run release:preflight`，再执行带 `--env-file /dev/null` 的 production D1 remote migration，成功后默认从 `dist/eruoo_server/wrangler.json` 执行同样带安全 env file、`--no-x-provision --strict` 的 deploy。任一尚未开始的步骤都必须被前置失败阻止。唯一例外是现有 Dashboard template 首次由仓库接管：取得 owner 对添加临时 Build variable、部署新精确 SHA 和 build 终态后删除变量的当次授权后，可设置 `PRODUCTION_WORKER_BOOTSTRAP_VERSION_ID` 为当前 Active template 的小写 UUID。脚本必须在 migration 前和 deploy 前执行第 21.2 节规定的两次最小远端检查与无冲突 Custom Domain changeset；只有全部成立时才允许本次 deploy 省略 `--strict`，并添加本次精确 SHA/UUID tag 与 message，不得使用 `--keep-vars` 或 `--force`。命令无论成功或失败都必须先完成有界的最小远端 postcondition 和最终 Git context 校验；命令报错不得转为成功，对账失败也不得声称远端回滚。变量无效、状态不符、无法读取或在运行中改变都必须 fail closed；远端接管后遗留的变量也必须在 migration 前停止。该临时变量不进入 `wrangler.jsonc`、GitHub Actions、runtime variables 或 Secret，不替代精确 SHA 的生产授权。测试必须验证 strict 与 bootstrap 两种命令顺序、唯一部署身份、command failure 后仍对账和 eventual consistency，并单独运行 `pnpm run bundle:check` 验证部署产物。真实执行 `deploy:production` 只能发生在本节规定的逐次生产部署授权之后。
...
- 首次接线当前存在 owner 已明确接受的临时例外：production trigger 选中的 Cloudflare 自动生成 build token 暂时保留平台自动加入的额外权限，另一个未选中的部署 token 暂留待后续决定。该例外不改变上一条的最终最小权限目标，也不得表述为已完成 least privilege；例外期间 non-production branch builds 必须关闭，token 不得复用于 GitHub 或其他项目。首次成功部署和验收后，收敛权限及处置未使用 token 仍需 owner 对具体外部变更另行授权。
- `deploy:production` 入口必须在执行 preflight、migration 和 deploy 的每一步前验证 `WORKERS_CI=1`、`WORKERS_CI_BRANCH` 精确等于 `production`，且 `WORKERS_CI_COMMIT_SHA`、checkout 的 `HEAD` 与 GitHub 上 `eruoo/server` 的 `refs/heads/production` 三者相同，同时要求源码工作区干净；Worker deploy 命令结束后必须再执行一次相同校验，bootstrap 路径必须先对账远端状态再做该校验，末次失败时 build 失败且不得打 tag。`WORKERS_CI_*` 只提供执行上下文，不是可信的平台身份证明；实际授权边界由远端分支 ruleset 和隔离的 production deploy token 提供。Workers Builds 构建父环境中的 `CLOUDFLARE_API_BASE_URL` 和 `WRANGLER_CI_GENERATE_PREVIEW_ALIAS` 只可作为不可信的平台输入存在，不得改变任何部署网络目标或启用 Preview Alias，并必须从 Git、preflight、Wrangler 和 bootstrap inspection 子进程中剥离；Wrangler 子进程使用 production/public 的默认 endpoint，Wrangler 与 bootstrap inspection 子进程明确固定 `WRANGLER_CI_GENERATE_PREVIEW_ALIAS=false`，bootstrap 的直接 API 检查固定使用 `https://api.cloudflare.com/client/v4`。入口必须拒绝旧别名 `CF_API_BASE_URL`、staging API、非 public compliance region，以及与 production Worker/account 不同的 Wrangler 覆盖；必须要求 trigger 提供非空 `CLOUDFLARE_API_TOKEN`，不得回退到 Wrangler 磁盘 OAuth 或全局凭证，并拒绝 Workers Builds 手工设置只供 GitHub Actions 使用的 `PRODUCTION_MIGRATION_BASELINE_SHA`。preflight 与 Git 校验子进程不得接收该 token；常规路径只有 D1 migration 与 Worker deploy 子进程可以接收生产 token，一次性 bootstrap 的最小只读探针也可使用同一 token。所有生产 Wrangler 命令必须固定 account/API environment/Worker name、禁用磁盘日志，并显式使用 `/dev/null` 作为 env file。任何 D1 migration 或 Worker upload 尝试后的失败都必须保留原始错误，明确提示远端可能已部分生效并禁止自动重试。上一个 production build 完成或明确失败前不得再次推进 `production`。
- 发布安全模型信任 GitHub 上受保护的 `production` 分支、Cloudflare 提供的构建容器和单写者操作窗口。clean worktree、exact SHA 与 preflight 用于阻止普通配置漂移和错误候选，不把同一 UID 恶意进程、受损构建容器或已执行恶意依赖视为脚本能够隔离的对象；出现这类怀疑时必须停止发布并重新建立可信构建。
- 每次发布都必须先确认最终 `main` commit 的 GitHub Actions `check` 为成功，再取得 owner 对该精确 commit 的生产部署授权，最后由 LoTwT 将 `production` 快进到同一 commit；只有这一受控分支更新才能触发 production deploy。Agent 不得更新 `production`，普通 `main` merge 或 CI 成功也不构成生产部署授权。GitHub ruleset 必须禁止 force push、把 `production` 更新权限限制给 LoTwT，并以无 bypass required status check 阻止未通过 CI 的 commit；配置 ruleset 和 production trigger 本身属于第 27 节外部操作门禁。
...
- LoTwT 将功能 PR squash merge 到 `main`；最终 squash commit 必须由 `main` push 触发的 GitHub Actions `check` 成功验证。生产发布使用第 22.3 节的独立 `production` 分支门禁，Agent 不得更新 `production`，也不得把普通代码合并、PR check 或 CI 成功解释为部署授权。
...
- 生产运行时 secret 只进入 Cloudflare Secret，生产 build token 只进入 Workers Builds 的受保护配置；GitHub CI 不配置生产 Secret 或 Cloudflare token。不同信任域不得复用高权限凭证。
