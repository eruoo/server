...
常规变更的执行时间目标：检查和构建 3 分钟，下载产物、必要 migration、部署与冒烟 2 分钟，合计 5 分钟左右。该目标尚未实测；不含 runner 排队、owner 操作等待、首次资源接线或重型专项验收。CI/发布任务各设 10 分钟上限，超时结束并报告阶段；若写入已开始，按结果未知处理，不能自动重跑。上限不是达到目标的证据。
