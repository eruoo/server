...
- **采集中**：闲置后首次查询（冷）分布。cron 已于 2026-09-04 14:5x UTC 切换为每小时（`0 * * * *`，版本 `a70e23da`），采 1 小时间隔冷查询。
  - 阶段 3（后续）：暂停 cron，人工间隔 6–24 小时后 curl 一次，采长闲置数据。
  - 阶段 1 的 `*/5` 保温对照数据（2026-09-04 14:16–14:5x UTC，约 8 条 cron 记录）已在 Dashboard Workers Logs，作为热基线对照。
...
- [ ] 阶段 2 冷查询采集（cron 已切换每小时，数据在 Dashboard 积累中，预计数小时后可读）。
