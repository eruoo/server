...
局部调整（2026-09-18，owner 指定切分）：AI 状态存储、有界清理与恢复规则先于 AI 能力开放合入——`ai_connections`、`ai_authorization_sessions`、`ai_models`、`ai_invocations` 四张表随 0002 迁移与每日清理、恢复规划器清理一起加入。PR 3（2026-09-19）继续交付固定 Codex 连接器、凭证加密与刷新、设备授权编排和模型发现；PR 4（2026-09-19）交付 Responses 请求子集校验与共用 SSE/JSON 协议解析，均仅在测试装配中启用；AI HTTP 路由、AI Key 配置档、管理页面仍未注册、不执行 AI handler，OpenAPI 不发布未来接口（审计事件枚举随新增的授权审计事件扩展）。表与连接器/协议代码的存在不代表 AI 服务可用；“未启用拒绝”对 AI 路径继续适用。
