...
- 2026-09-12 owner 按江苏无锡经新加坡、日本、美国代理访问的实际场景确认新目标，替代原 100ms / 600ms 门槛；历史超标与失败记录保留，不追溯改写为通过。TTFB 包含客户端网络与代理成本，不能减去 health 延迟后再判定。
- 每个指定代理节点的 warm JWE hit 与 warm D1 miss 各至少 30 个请求：JWE 的 D1 I/O 为 0、TTFB p95 ≤ 500ms；仅携带 Session Cookie 的 D1 读取 TTFB p95 ≤ 1,000ms。分位数采用最近秩法，并报告 max、HTTP 错误及超时数量；远端无法直接计数 I/O 时，须同时引用当前代码的原生 workerd/D1 测试，不把 Cookie 命中推断写成远端零 I/O 实测。
- cold 场景至少 5 次独立闲置，每次首次请求 TTFB ≤ 2,000ms；逐个报告，不假定闲置等于 isolate 已回收。超标时分析网络/D1/初始化，不以平均值或加大样本量掩盖原有失败。仅文档或发布工具变更、运行时与数据库配置不变时，可引用同代码专项结果并明确版本差异；不能称为新版本冷启动复测。
...
局部调整（2026-09-18，owner 指定切分）：AI 状态存储、有界清理与恢复规则先于 AI 能力开放合入——`ai_connections`、`ai_authorization_sessions`、`ai_models`、`ai_invocations` 四张表随 0002 迁移与每日清理、恢复规划器清理一起加入。PR 3（2026-09-19）继续交付固定 Codex 连接器、凭证加密与刷新、设备授权编排和模型发现；PR 4（2026-09-19）交付 Responses 请求子集校验与共用 SSE/JSON 协议解析；PR 5（2026-09-19）交付网络调用编排（凭证阶段、上游调用与预算、401 重发、终态落库）；PR 6（2026-09-19）开放 AI Key 配置档（`purpose=ai`、`modelIds`、按连接 UUID 的模型许可与网关白名单，五个 api-key 契约随之更新）并交付模型授权解析。该档位由既有 api-key 网关直接服务（无环境开关）：在远端 AI 表迁移与部署配置落地前，`purpose=ai` 创建会因缺少 AI 表以 503 失败关闭，不会产生半成品 Key；PR 7（2026-09-19）注册 §6.1 管理端点与 §6.2 调用端点（含准入、`AI_RATE_LIMITER`、读体上限、recent-auth 与 OpenAPI 契约）并落地部署配置声明（`AI_CREDENTIAL_KEYS`、`AI_RATE_LIMITER`、`enable_request_signal`）；管理页面已交付（2026-09-19，PR 8/PR 9，见实施记录 §4.20）；生成契约中 14 个 AI 操作的响应 schema 已补齐，请求体 schema 按“认证后读 body”的安全顺序保持由 handler 校验、不注册到框架层（取舍理由见 §4.20）。staging 已部署并实测（2026-09-19，实施记录 §4.21）：0003 迁移应用、真实设备授权与含 AI 数据的隔离恢复演练通过；模型目录请求返回 403（具体原因尚未确认，出口或边缘防护为待验证假说），该连接器当前不可用，AI Key 无法授予目录内模型，真实推理链路未验收，AI 上线门槛未通过（owner 决策维持现状并记录），production 的实际行为需在部署后单独验收。表、连接器、协议、编排与 Key 档代码的存在不代表 AI 服务可用；“未启用拒绝”对 AI 路径继续适用。
...
本轮交付服务端、Web、测试及发布脚本；具体覆盖、已执行的平台操作与剩余验收项统一见 [implementation.md](implementation.md)。真实平台备份/恢复、回退、第三方登录与性能测试仍按各自证据判断，不能用本地通过或单项成功代替整体上线验收。
...
AI 后续调整（2026-09-22）：DeepSeek 官方 API Key 替代 Codex OAuth，0004 一次性清理旧 AI 状态。当前范围与验收口径以 [AI 规格](ai-service.md) 为准，上述设备授权与刷新记录为历史验收，不作为新链路上线证据。
