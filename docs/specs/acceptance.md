...
局部调整（2026-09-18，owner 指定切分）：AI 状态存储、有界清理与恢复规则先于 AI 能力开放合入——`ai_connections`、`ai_authorization_sessions`、`ai_models`、`ai_invocations` 四张表随 0002 迁移与每日清理、恢复规划器清理一起加入，但 AI HTTP 路由、AI Key 配置档、管理页面与上游连接器仍未注册、不执行 AI handler，OpenAPI 不发布未来接口。表的存在不代表 AI 服务可用；“未启用拒绝”对 AI 路径继续适用。
