...
2026-09-19 实测（staging，实施记录 §4.21）：设备登录一侧成立（真实设备授权完成，账号链路经 owner 本机 codex 复核正常）；Worker 侧推理协议不成立——chatgpt.com 后端对 Cloudflare Worker 出口请求返回 403 边缘挑战，令牌未被评估，模型目录无法建立。该前提当前按上线门槛阻断处理：不伪造第一方身份、不自动改用付费 API、不新建中继组件（owner 决策维持现状并记录）；出口通道的任何变更需 owner 另行决策与授权后重新验收。
...
| 模型目录        | GET `https://chatgpt.com/backend-api/codex/models?client_version=…`，Bearer + `chatgpt-account-id`；响应 `{models:[{slug, display_name, supported_reasoning_levels, supported_in_api, visibility, …}]}`；目录请求 401 按 codex-rs 会话语义做一次强制刷新与一次重发（PR #45，2026-09-19）                                                                                                                                 | codex-rs models 端点；**2026-09-19 实测**：Worker 出口访问该端点被 Cloudflare 边缘 403 挑战拒绝（令牌未被评估，见 §13 上线门槛记录）；同头型请求自非 Worker 网络得到 401 鉴权应答                              |
...
| 上游标识头      | `originator: eruoo`、`user-agent: eruoo/1`（第三方 originator 为参考模式：OpenCode 发送 `opencode`）                                                                                                                                                                                                                                                                                                                     | OpenCode；**2026-09-19 实测**：头型本身不触发拒绝（本机同头型无凭证请求得 401 鉴权应答）；Worker 出口 403 为环境级阻断（出口 IP + workerd TLS 指纹），owner 本机 codex app/CLI 同账号正常                      |
...
- 2026-09-19 staging 实测（实施记录 §4.21）：真实设备授权、授权会话恢复、重新授权与取消通过；首次凭证有效期约 10 天；目录请求 401 恢复语义已实现并经独立 review 合入（PR #45）；**模型发现与推理调用被 chatgpt.com 对 Worker 出口的 403 边缘挑战阻断**（owner 本机 codex 正常，阻断归因于出口环境），§13 上线门槛条件触发，owner 决策维持现状并记录；含 AI 数据的隔离恢复演练通过。真实推理链路、真实令牌刷新、真实流内错误分类与 `AI_CREDENTIAL_KEYS` 轮换仍待出口问题解决后验证。
