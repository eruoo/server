...
| `CF_ACCOUNT_ID`                                                    | 已从当前生产 Cloudflare account 确认；部署权威值维护在 `wrangler.jsonc#env.production.account_id`，`vars.CF_ACCOUNT_ID` 是传给 Worker 的精确 mirror，默认本地环境仍保持为空。                                          |
...
`check` 依次覆盖格式、lint、secret scan、生产依赖安全公告、Wrangler binding 类型、TypeScript、Vue SFC、Better Auth schema drift、组件测试、真实 workerd/D1 集成测试、Playwright，以及完整 `release:preflight`（OpenAPI drift、production D1 migration 路径与 ledger、可信 Git 基线上的 append-only migration 契约、生产构建、生成配置核心契约、Wrangler dry-run 和本地 startup profile）。本机执行前使用 `pnpm exec playwright install chromium` 安装浏览器；GitHub Actions 的 Ubuntu runner 使用 `pnpm exec playwright install --with-deps chromium` 安装浏览器和系统依赖，然后运行同一个 `pnpm run check`。

`.github/workflows/check.yml` 只监听指向 `main` 的 PR 和 `main` push。PR run 提供合入前反馈，最终 squash commit 必须由 `main` push 再次运行并以稳定 job 名 `check` 成功，才能进入 `production`。该 workflow 固定 Node.js 版本、从 `package.json#packageManager` 读取 pnpm，并使用只读仓库权限；它在执行仓库代码前检出完整候选历史、显式刷新受保护的 `production` ref，并将其精确 commit SHA 仅作为本次 `check` 的 migration 基线。它没有生产凭证、远端 migration 或 deploy，Cloudflare Workers Builds 也不重复 Playwright 和完整 `check`。
...
`pnpm run release:preflight` 也是只读检查，会严格要求已提交的 production D1 ID、顶层 Cloudflare `account_id` 与 runtime mirror、正式域名、required-secret manifest、migration contract 和生成后的 production 核心配置一致。migration 文件名和恢复规划复用同一 validator；GitHub Actions 要求受保护的 `production` tip 是候选 `HEAD` 可读的 first-parent 祖先，并按时间顺序读取完整候选路径。校验从 production 快照开始，只用符合 append-only、逐字节不变和序号递增规则的候选快照推进可信状态，最终候选必须相对最近的合法快照继续追加；因此失败或取消的中间提交不能被后续无关提交洗白，明确恢复到最近合法状态后则可重新通过。`PRODUCTION_MIGRATION_BASELINE_SHA` 只允许由 GitHub Actions 注入；变量缺失、格式错误、祖先关系异常或路径中任一 Git 对象无法读取时 fail closed。本地检查没有该变量时只使用直接父提交提供快速反馈；Workers Builds 始终明确告警本次 append-only 检查不具权威性，并只消费已经通过 GitHub Actions `check` 中的 production-history 门禁与受保护 `production` exact-SHA 门禁的同一提交。其 Wrangler dry-run/startup 子命令显式使用 `/dev/null` 作为 env file，不读取本地 `.env*`。它只验证 Secret 名称清单，不读取远端 Secret 值；任何本地成功都不构成生产部署授权。
...
`pnpm run deploy:production` 只能在最终 `main` commit 的 GitHub Actions `check` 已成功、无 bypass status gate 允许同一 SHA 进入 `production`，且 owner 对该 SHA 给出当次生产部署授权后执行。它会在每个步骤前确认 Workers Builds branch 为 `production`，且平台注入的完整 commit SHA、checkout 的 `HEAD` 与 GitHub 上受保护的远端 `production` ref 三者一致，同时要求源码工作区干净，然后按 `release:preflight`、production D1 migration、Worker deploy 的顺序执行；常规发布使用生成的 production 配置和 `--strict`，任一步失败都会停止尚未开始的后续步骤。唯一的一次性 Dashboard template 接管例外会在 migration 前和 deploy 前完成最小只读检查与无冲突 Custom Domain changeset，随后执行带唯一 tag/message 的 non-strict deploy；deploy 命令成功或失败后都先做有界的最小远端对账，完整约束见[生产发布](./releasing.md#一次性-dashboard-模板接管)。入口要求非空 build token、固定 account/API environment/Worker name，并禁止 Wrangler 磁盘凭证回退；preflight 与 Git 校验不接收 token，常规路径只有 D1/deploy 子进程接收 token，bootstrap 的最小只读探针也可使用同一 token。生产 Wrangler 命令禁用磁盘日志并使用 `/dev/null` 作为 env file。发布流程信任受保护的 `production` 分支、Cloudflare 构建容器和单写者窗口；同一 UID 恶意进程或已受损构建依赖不属于脚本的隔离目标。任何 migration 或 upload 尝试后的失败都意味着远端可能已部分生效，禁止自动重试。上一个 production build 结束前不得再次推进分支。该命令不是本地验收命令；Workers Builds 必须使用目标账号范围内、同时具备严格部署所需权限和 D1 Edit 的自定义最小权限 token。
