...
- [ai-service.md](specs/ai-service.md)：面向多个应用的通用 AI 接入、凭证管理与调用架构；首版 Codex OAuth。AI 设计的唯一来源；其存储、清理与恢复、连接器、协议、调用编排与 AI Key 配置档切片已实施（本地合成验证），AI HTTP 路由已注册（本地合成验证）；管理界面未交付。
