...
实现及本地验证结果见 [implementation.md](specs/implementation.md)，不代表获得部署授权。2026-09-09 最新需求已纳入：原生滚动续期、数据从新空库开始、GitHub Actions 产物发布、约 5 分钟常规执行时间目标，以及先做 Web、Desktop 延后、不维护旧接口兼容；依据见架构 Q3–Q7。owner 新指令覆盖冲突的历史条款，其余规则继续有效。平台实测与接口字段快照仍分别维护，本次验收服务端和 Web，实际 Desktop 联调延后。
