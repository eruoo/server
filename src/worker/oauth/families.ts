...
> = async (context, next) => {
  ...
  if (
    ...
    operation = "revoke"
    rawRefreshToken = stripAccessTokenAuthorizationScheme(
      exactlyOne(values, "token") ?? "",
    )
  } else {
    await next()
    return
  ...
  if (operation === "revoke") {
    ...
    const created = await tombstoneRefreshFamily(
      context.env.DB,
      family,
      Date.now(),
    )
    ...
    context.res = successfulRevocationResponse()
    return
  }
  if (
  ...
  let reuseWinner = false
  if (isReuseOutsideRetryWindow(family, observedAt)) {
    reuseWinner = await tombstoneRefreshFamily(
      context.env.DB,
      family,
      observedAt,
    )
...
export async function revokeClientFamilies(
  ...
  const results = await database.batch([
    database
      .prepare(
        `INSERT OR IGNORE INTO oauthRefreshTokenFamilyRevocation (authorizationCodeId, clientId, userId, revokedAt) SELECT DISTINCT authorizationCodeId, clientId, userId, ? FROM oauthRefreshToken WHERE userId=? AND clientId=? AND authorizationCodeId IS NOT NULL`,
      )
      .bind(now, subjectId, clientId),
    database
      .prepare(
        "UPDATE oauthRefreshToken SET revoked=? WHERE userId=? AND clientId=? AND revoked IS NULL",
      )
      .bind(new Date(now).toISOString(), subjectId, clientId),
    database
      .prepare("DELETE FROM oauthConsent WHERE userId=? AND clientId=?")
      .bind(subjectId, clientId),
  ])
  const revokedRefreshTokenCount = results[1]?.meta.changes
  const deletedConsentCount = results[2]?.meta.changes
  if (
    typeof revokedRefreshTokenCount !== "number" ||
    typeof deletedConsentCount !== "number"
  )
    throw new Error("Invalid revocation result")
  return { revokedRefreshTokenCount, deletedConsentCount }
}
