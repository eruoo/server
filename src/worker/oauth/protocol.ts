...
function validateScopeValues(
  ...
  if (
    values.length > 1 ||
    (values.length === 1 && !isCanonicalOAuthScope(values[0]!))
  ) {
    return {
      error: "invalid_scope",
      errorDescription:
...
function redirectAuthorizationError(
  ...
  target.searchParams.append("iss", context.env.APP_ORIGIN)
...
export const validateOAuthTokenRequest: MiddlewareHandler<AppBindings> = async (
  ...
  if (grantType !== null && grantType !== grantType.trim()) {
    return oauthError(context, {
      error: "invalid_request",
      errorDescription:
        "grant_type must not contain leading or trailing whitespace",
    })
  }
...
function normalizeMetadata(
  ...
  const config = { appOrigin: context.env.APP_ORIGIN }
...
export async function serveOAuthMetadata(
  ...
  const auth = getRequestAuth(context)
