...
- **实施授权边界**：可自主推进的本地工作与必须由 owner 明确授权的外部操作，以 [重构规格第 7 节（执行配置）](./docs/specs/refactoring.md)与 [重设计规范盘点 §27](./docs/specs/redesign-norms.md)（流程硬约束：外部操作需 owner 当时授权，重设计期间继续适用）为准。
- **v2 事实来源**：平台实测数据见 [platform-facts.md](./docs/specs/platform-facts.md)；旧 foundation.md 为参考档（非权威）。
